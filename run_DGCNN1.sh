...
sortpooling_k=0.9  # If k <= 1, then k is set to an integer so that k% of graphs have nodes less than this integer
...
if [ ${fold} == 0 ]; then
  ...
  for i in $(seq 1 10)
  do
    CUDA_VISIBLE_DEVICES=${GPU} python3.6 train_dgcnn.py \
        -seed 1 \
        -data $DATA \
        -fold $i \
